...
def main() -> int:
    ...
    for pid, mode, prompt in ADVERSARIAL:
        ...
        try:
            ...
            print(f"  ! {pid}: {str(exc)[:80]}")
            continue
        rows[pid] = {"truth": UNKNOWN, "got": got, "style": mode,
                     "confidence": conf, "why": why, "kind": "adversarial"}
        mark = "ok  " if got == UNKNOWN else "WRONG"
...
def _report(rows: dict[str, dict], reader: str, unreachable: int) -> None:
    ...
    for v in adversarial.values():
        by_mode[v["style"]].append(v["got"] == UNKNOWN)
    if by_mode:
    ...
    if adversarial:
        refused = sum(v["got"] == UNKNOWN for v in adversarial.values())
